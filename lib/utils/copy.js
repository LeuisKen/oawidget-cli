...
function writeFiles(dirSrcPath, dirTarPath, data, cmptName) {
    ...
    files.forEach(function (item) {
        ...
        if (cmptName) {
            ...
            dirTarPath = path.join(dirTarPath, cmptName);
...
function run(cmptName) {
    ...
    // cp通用脚手架文件
    shell.cp('-r', path.join(TEMPLATE_SRC_PATH, '*'), '.');
    shell.cp('-r', path.join(TEMPLATE_SRC_PATH, '.babelrc'), '.');
    shell.cp('-r', path.join(TEMPLATE_SRC_PATH, '.ignore'), '.');
    shell.mv('.ignore', '.gitignore');
    shell.mkdir('-p', path.join('preview', 'App.js'));
    shell.mkdir('-p', path.join('preview', 'index.html'));
    shell.mkdir('-p', path.join('preview', 'index.js'));
